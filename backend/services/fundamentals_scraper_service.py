"""
Gemini UI Scraper Service for Fundamentals
Exact same functionality as geminiCompanyProfile.py but for fundamentals data
"""
...
SESSION_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "x_browser_session_gemini_fundamentals")
HEADLESS = False
CHROME_PATH = None
MOUSE_STEP_MS = 6
SELECTOR_MEMORY_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), "working_selectors_fundamentals.json")
STABILIZE_SECONDS = 7
        ...
        try:
            ...
            for el in els:
                try:
                    ...
                    prompt_indicators = [
                        "Provide a complete and comprehensive fundamentals",
                                ...
                                if json_text.startswith('{') and user_prompt_clean not in json_text and len(json_text) > MIN_ACCEPT_CHARS:
                                    try:
                                        ...
                                        if isinstance(test_parse, dict):
                                            expected_keys = ['CoreMetrics', 'Signals']
    ...
    if response_stripped.startswith('"') and '{' in response_stripped:
        try:
            ...
            if isinstance(parsed_string, str):
                ...
                if isinstance(parsed, dict) and any(key in parsed for key in ['CoreMetrics', 'Signals']):
                    return parsed
...
def _run_playwright_sync_fundamentals(ticker: str) -> Optional[Dict]:
    ...
    try:
        return loop.run_until_complete(_fetch_fundamentals_async(ticker))
    finally:
        loop.close()


async def fetch_fundamentals_from_gemini(ticker: str) -> Optional[Dict]:
    """
    Launch Gemini browser, send fundamentals query, collect response.
        ...
        try:
            ...
            if not isinstance(loop, asyncio.ProactorEventLoop):
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(_run_playwright_sync_fundamentals, ticker)
    ...
    return await _fetch_fundamentals_async(ticker)


async def _fetch_fundamentals_async(ticker: str) -> Optional[Dict]:
    """
    Internal async function to fetch fundamentals from Gemini.
    This is called by _run_playwright_sync_fundamentals for Windows compatibility.
    """
    ...
    async with async_playwright() as p:
        ...
        query = f"""Provide a complete and comprehensive fundamentals analysis for stock ticker {ticker} in JSON format. 
...
{{
  "CoreMetrics": {{
    "Revenue": {{
      "current": "current revenue value",
      "yoy_growth": "year-over-year growth percentage",
      "qoq_growth": "quarter-over-quarter growth percentage",
      "trend": "increasing/decreasing/stable",
      "history": ["revenue history if available"]
    }},
    "EPS": {{
      "current": "current EPS value",
      "trend": "positive/negative/volatile",
      "history": ["EPS history if available"],
    ...
    "FreeCashFlow": {{
      "current": "current free cash flow value",
      "trend": "increasing/decreasing/stable",
      "history": ["FCF history if available"],
    ...
    "Margins": {{
      ...
      "trends": {{
        "gross_trend": "trend description",
        "operating_trend": "trend description",
        "net_trend": "trend description"
      }}
    }},
    "Filings": {{
      "10-K": ["list of 10-K filing dates and links"],
      "10-Q": ["list of 10-Q filing dates and links"],
      "8-K": ["list of 8-K filing dates and links"],
      "S-1": ["list of S-1 filing dates and links if available"]
    }}
  }},
  "Signals": {{
    "FundamentalsTrend": {{
      ...
      "assessment": "overall fundamentals assessment"
    }},
    "InsiderActivity": {{
      "recent_buys": ["list of recent insider purchases"],
      "recent_sells": ["list of recent insider sales"],
      "net_activity": "buy/sell/neutral",
      "conviction_score": "conviction score if available",
      "insider_ownership": "insider ownership percentage"
    }},
    "InstitutionalActivity": {{
      "13F_filings": ["list of recent 13F filing dates"],
      "accumulation": "increasing/decreasing/stable",
      "top_holders": ["list of top institutional holders"],
      "ownership_trend": "accumulating/distributing/stable",
...
Provide factual, detailed, and comprehensive fundamentals information about {ticker}. Return ONLY the JSON object, nothing else."""

        print(f"💬 Sending fundamentals query for {ticker}...")
        input_box = None
        ...
        await browser.close()
        print(f"✅ Fundamentals extracted for {ticker}")
